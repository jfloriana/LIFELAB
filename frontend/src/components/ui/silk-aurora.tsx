...
export function SilkAurora({
  ...
  const settings = React.useMemo(
    () => ({
      ...
      accentColor,
      speed,
      intensity: lightOverlays ? 1.5 : intensity,
      grain: lightOverlays ? 0 : grain,
      vignette: lightOverlays ? 0 : vignette,
      mouseInfluence,
      interactive,
    }),
    [
  ...
  const fallbackContent = (
    <div
      className={cn(
        ...
        lightOverlays ? "bg-white" : "bg-[#050507] text-white",
        className,
      )}
  ...
  return (
    <WebGLErrorBoundary fallback={fallbackContent}>
      ...
      >
        ...
        {lightOverlays ? (
          <>
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_72%_34%,rgba(8,145,178,0.12),transparent_40%),radial-gradient(circle_at_18%_74%,rgba(34,211,238,0.08),transparent_40%)]" />
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-white/40 to-transparent" />
          </>
        ) : (
          <>
